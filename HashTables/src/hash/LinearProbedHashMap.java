 ...
 */
public class LinearProbedHashMap<K, V> extends ArrayHashMap<K, V, Entry<K, V>> {
	...
	private int calculatePositionPassive(K key) {
		...
		} while (originalPos != pos /* haven't looped */);
	...
	@Override
	public V get(K key) {
		final Entry<K, V> at = list[calculatePositionPassive(key)];
